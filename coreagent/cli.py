...
def _parse_args():
    ...
    p.add_argument("-m", "--model", help="Model name (default: $COREAGENT_MODEL or gpt-5.5)")
...
def main():
    ...
    if not config.api_key:
        ...
        console.print(
            "Set one of: OPENAI_API_KEY, DEEPSEEK_API_KEY, or COREAGENT_API_KEY\n"
            "\nExamples:\n"
            "  # OpenAI\n"
            ...
            "\n"
            "  # Ollama (local)\n"
            "  export OPENAI_API_KEY=ollama OPENAI_BASE_URL=http://localhost:11434/v1 COREAGENT_MODEL=qwen2.5-coder\n"
        )
        sys.exit(1)
    ...
    # 判断是否为一次性执行模式（非交互模式），执行一轮任务之后就退出
    if args.prompt:
...
def _repl(agent: Agent, config: Config):
    """交互式 读取-执行-打印 循环。"""
    console.print(Panel(
        f"[bold]CoreAgent[/bold] v{__version__}\n"
    ...
    # 设置命令行历史记录
    hist_path = os.path.expanduser("~/.coreagent_history")
    history = FileHistory(hist_path)

    # Enter 按键提交，Escape+Enter 按键插入换行符（用于粘贴代码块等）。
    kb = KeyBindings()
    ...
    while True:
        ...
        if user_input == "/save":
            ...
            console.print(f"Resume with: coreagent -r {sid}")
            continue
        if user_input == "/diff":
        ...
        try:
            ...
            # 如果响应是流式的，已经在 on_token 回调中打印了，所以这里不再打印
            if streamed:
                print()
            else:
                # 响应未被实时流式传输则需要打印（在工具调用之后才出现）
